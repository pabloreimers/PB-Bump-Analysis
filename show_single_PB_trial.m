...
for i = 1:length(all_files)
    ...
    if ~isfile([fileparts(all_files(i).folder),'\mask.mat'])
        ...
        figure(1); clf; imagesc(sum(imgData,3)); colormap(parula); axis equal tight; drawnow;
        mask = roipoly();
...
for i = length(all_data):length(all_files)
    ...
    background_F = nan(1,1,size(imgData,3));
    curr_img = smoothdata(imgData,3,im_type{1},im_win{1});
    for f = 1:size(imgData,3)
        curr_frame = curr_img(:,:,f);
        background_F(f) = median(curr_frame(~mask));
    end

    all_data(i).im = process_im(imgData - background_F, im_win, im_type, mask, n_centroid, f0_pct);
