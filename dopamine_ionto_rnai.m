...
%% load in data
base_dir = 'Z:\pablo\dopamine_ionto\lpsp_thrnai\'; %uigetdir(); %
...
for i = length(all_data):length(all_files)
    ...
    try
        ...
        all_data(i).atp.d = nan(size(all_data(i).im.d));
    end
...
tmp(:,:,1) = all_data(i).atp.d*1;
tmp(:,:,2) = all_data(i).im.d*1;

figure(1); clf
a0 = subplot(12,1,1); hold on
...
a1 = subplot(12,1,2:4); hold on
image(all_data(i).ft.xb,unwrap(all_data(i).im.alpha),tmp)
...
pos = [pos(1)+pos(3)+.01,pos(2),.05,pos(4)/2];
ax = axes('Position',pos,'Color','none','XAxisLocation','top');
histogram(all_data(i).im.mu,-pi:.1:pi,'Orientation','horizontal','edgeColor','none','Normalization','probability')
hold on
histogram(-all_data(i).ft.cue,-pi:.1:pi,'Orientation','horizontal','edgeColor','none','Normalization','probability','FaceColor','c')
box(ax,'off')
