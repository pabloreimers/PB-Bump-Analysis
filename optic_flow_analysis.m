...
all_fs = vertcat(data.r_speed{fly_food});
all_cs = vertcat(data.c_speed{fly_food});

os_vec = round(0:.1:1,1);
f_cell = cell(size(os_vec));
ind_fits = cell(size(os_vec));


figure(1); clf; hold on
colororder(cool(length(os_vec)))
for i = 1:length(os_vec)
    f_cell{i} = fit(all_fs(all_cs==os_vec(i)),all_df(all_cs==os_vec(i)),'poly3');
    ind_fits{i}  = cellfun(@(x,y,z)...
                         (fit(x(z==os_vec(i)),y(z==os_vec(i)),'poly3')), ...
                         data.r_speed,data.dff_tot,data.c_speed, ...
                         'UniformOutput',false);
    plot(0:.1:3,feval(f_cell{i},0:.1:3),'linewidth',2)
end
ylabel('fit dF/F')
...
set(gcf,'color','none')
set(get(gcf,'Children'),'color','none','YColor','w','XColor','w')
legend(num2str(os_vec'),'Location','Southeastoutside','textcolor','w','edgecolor','w')
...
tiledlayout(rows,cols)
for i = 1:n
        subplot(rows,cols,i); hold on; colororder(cool(length(ind_fits)))
        cellfun(@(x)(plot(0:.1:3,feval(x{i},0:.1:3),'linewidth',1)),ind_fits)
        h = ylim;
        ylim([0,min(h(2),10)])
end
set(gcf,'color','none')
...
fs_aligned = nan(n,size(data.c_speed{1},1));
...
for i = 1:n
    ...
    if delay_vec(i) > 0
       ...
       fs_aligned(i,1:end-delay_vec(i)+1) = data.r_speed{i}(delay_vec(i):end);
       ...
       fs_aligned(i,-delay_vec(i)+1:end) = data.r_speed{i}(1:end-delay_vec(i):end);
...
h = ylim;
patch([x,fliplr(x)],(h(2)-h(1))*[c,zeros(size(c))]+h(1),[0,.5,1],'FaceAlpha',.5)
plot(x,y,'w')
...
subplot(4,2,[3,5,7]); hold on
x = data.xf{1};
y = mean(fs_aligned(fly_food,:),1);
s = std(fs_aligned(fly_food,:),1)./sqrt(sum(~isnan(fs_aligned(fly_food,:)),1));
idx = ~isnan(x.*y.*s);
x = x(idx);
y = y(idx);
s = s(idx);
