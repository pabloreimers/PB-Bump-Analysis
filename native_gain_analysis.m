...
for i = 1:n
    ...
    if pb_flag
        a = plot(xf{i},mu{i}+2*pi,'-','linewidth',2,'Color',[1,1,1]);
        a.YData(abs(diff(a.YData)) > pi) = nan;
        a.YData(rho{i}<rho_thresh) = nan;
        b = plot(xf{i},tmp_cue+2*pi,'-','linewidth',2,'Color',[.75,0,.75]);
        b.YData(abs(diff(b.YData)) > pi) = nan;
    end
...
figure(11); clf
for i = 1:n
    subplot(n,6,(i-1)*6+[1:5]);
    tmp = circ_dist(mu{i},-cue{i});
    tmp(rho{i}<rho_thresh) = nan;
    a = plot(xf{i},tmp);
    a.YData(abs(diff(a.YData))>pi) = nan;
    axis tight

    [counts,edges] = histcounts(tmp,'binwidth',.1);
    subplot(n,6,i*6)
    barh(edges(1:end-1),counts,'EdgeColor','none')
    text(max(counts),min(edges),sprintf('Offset Var: %.2f',circ_var(tmp(~isnan(tmp)))),'VerticalAlignment','bottom','HorizontalAlignment','right')


end

%% Other figures (finding gain and such)
...
% reps = 1;
% win_size = 120; %in seconds for piecewise optimization
...
% [params(t,:,j),fval(t,1,j)] = fmincon(obj_fun,x0(j,:),[],[],[],[],lb,ub,[],opts);
...
% 
% for i = 1:n
%     [~,idx] = sort(fval_cell{i},1,'ascend');
%     %subplot(n,cols,2*i - 1)
%     subplot(2,1,1); hold on
%     scatter(reshape(idx,1,[]), reshape(params_cell{i}(:,1,:),1,[]),'filled')
%     xticks([])
% 
%     %subplot(n,cols,2*i)
%     subplot(2,1,2); hold on
%     scatter(reshape(idx,1,[]),reshape(params_cell{i}(:,2,:),1,[])*fr*1000,'filled')
%     xticks([])
% end
% subplot(2,1,1)
% legend(tmp_labels)
% %title(subplot(rows,cols,1),'Sorted Fit Gain')
% %title(subplot(rows,cols,2),'Sorted Fit Lag')
% 
% fontsize(gcf,20,'pixels')
% shg
% 
...
% figure(13); clf
% for i = 1:n
% %[~,idx] = sort(fval_cell{i},'ascend');
% %x = squeeze(params_cell{i}(idx(1),:));
% fr = 1/60;
% x = [.7,0];
% lag = ceil(x(2));
% 
...
function loss = loss_fun(x,theta,mu,rho)
    ...
    idx = rho > .1; %create an index to only take strong estimates
    tmp1(~idx) = nan;
    tmp2(~idx) = nan;    
