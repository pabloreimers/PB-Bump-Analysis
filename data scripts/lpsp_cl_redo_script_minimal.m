...
for i = 1:length(all_files)
    ...
    tmp2 = dir([fileparts(all_files(i).folder),'\*ficTracData_DAQ*']);
...
inc_idx  = false(length(all_data),1);
...
for i = 1:length(all_data)
    ...
    dark_idx(i) = ~logical(mod(str2num(tmp2{2}),2));
...
for i = unique(ind)'
    subplot(2,length(unique(ind)),i)
    plot(X(ind==i & inc_idx,:)','o-','Color',.8*[1,1,1],'linewidth',2); hold on
    errorbar(mean(X(ind==i & inc_idx,:),1),std(X(ind==i & inc_idx,:),[],1)./sqrt(sum(~isnan(X(ind==i & inc_idx,:)),1)),...
        'ro-','linewidth',4);
    ylabel({'R-squared','(dff and kinematic)'})
    xticks(1:3); xticklabels({'forward','rotational','joint'});xlim([.5,3.5])
    title(group_order(i),'Color',c)
    text(max(xlim),0,sprintf('n = %i',sum(ind==i & inc_idx)),'color','w',...
...
tmp(~inc_idx) = nan;
tmp = reshape(tmp,2,[]);
plot(reshape(ind,2,[]),tmp,'o-','Color',.8*[1,1,1]); hold on;
errorbar([1,2],mean(tmp,2,'omitnan'),std(tmp,[],2,'omitnan')./sqrt(sum(~isnan(tmp),2)),...
    'ro-','linewidth',2);
xticks(unique(ind)); xticklabels(group_order); axis padded
...
for i= 1:n
    ...
    figure(2)
    subplot(rows,cols,i)
    scatter(fly_vel(idx),bump_vel(idx),'filled','markerfacealpha',.2)
    ...
    figure(4)
    xb = linspace(all_data(i).ft.xf(1),all_data(i).ft.xf(end),size(all_data(i).im.z,2));
    subplot(n/2,2,i)
...
figure(2); clf;
subplot(3,1,1)
plot(all_data(i).ft.xb,mean(all_data(i).im.d,1),'w','linewidth',2)
ylabel('\DeltaF/F','rotation',0)

subplot(3,1,2)
plot(all_data(i).ft.xf,abs(all_data(i).ft.r_speed),'c','linewidth',2)
ylabel({'roational speed','rad/s'},'rotation',0)

subplot(3,1,3)
plot(all_data(i).ft.xf,all_data(i).ft.f_speed,'r','linewidth',2)
ylabel({'forward speed','mm/s'},'rotation',0)
