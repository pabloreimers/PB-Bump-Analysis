...
%% load in data
base_dir = 'Z:\pablo\lpsp_gain_change\'; %uigetdir(); %
...
nexttile; hold on
tmp = nan(length(all_data),1);
for i = 1:length(all_data)

        scatter(speed_slope{i},amp_slope{i});
        idx = ~isnan(speed_slope{i}) & ~isnan(amp_slope{i});
        tmp(i) = speed_slope{i}(idx)' \ amp_slope{i}(idx)';
end
plot(tmp','Color',[0,0,0,.1])
xlim([0,4])
xlabel('speed slope')
