...
%% for each fly, find the optimal lag

opt_lags = nan(length(all_data),1);
opt_r     = nan(length(all_data),1);
rho_thresh = .2;
r_thresh = .1;
...
for i = 1:length(all_data)
    ...
    for k = 1:length(lags)
        ...
        x = dr(1:end-lag);
        y = dm(1+lag:end);
        idx = ~isnan(x) & ~isnan(y) & x > r_thresh;
...
tmp = 1:length(all_data);

a1 = subplot(2,1,1); hold on
a2 = subplot(2,2,3); hold on
a3 = subplot(2,2,4); hold on

idx = ~empty_idx & ~mcherry_idx & ~vglut_idx & ~dark_idx & walk_idx;
scatter(a1,tmp(idx),opt_lags(idx))
scatter(a2,1*ones(sum(idx),1),opt_lags(idx),'filled','markerfacealpha',.1)
scatter(a3,1*ones(sum(idx),1),opt_r(idx),'filled','markerfacealpha',.1)
idx = empty_idx & ~mcherry_idx & ~vglut_idx & ~dark_idx & walk_idx;
scatter(a1,tmp(idx),opt_lags(idx))
scatter(a2,2*ones(sum(idx),1),opt_lags(idx),'filled','markerfacealpha',.1)
scatter(a3,2*ones(sum(idx),1),opt_r(idx),'filled','markerfacealpha',.1)
idx = ~empty_idx & mcherry_idx & ~vglut_idx & ~dark_idx & walk_idx;
scatter(a1,tmp(idx),opt_lags(idx))
scatter(a2,3*ones(sum(idx),1),opt_lags(idx),'filled','markerfacealpha',.1)
scatter(a3,3*ones(sum(idx),1),opt_r(idx),'filled','markerfacealpha',.1)
idx = ~empty_idx & ~mcherry_idx & vglut_idx & ~dark_idx & walk_idx;
scatter(a1,tmp(idx),opt_lags(idx))
scatter(a2,4*ones(sum(idx),1),opt_lags(idx),'filled','markerfacealpha',.1)
scatter(a3,4*ones(sum(idx),1),opt_r(idx),'filled','markerfacealpha',.1)
idx = empty_idx & ~mcherry_idx & vglut_idx & ~dark_idx & walk_idx;
scatter(a1,tmp(idx),opt_lags(idx))
scatter(a2,5*ones(sum(idx),1),opt_lags(idx),'filled','markerfacealpha',.1)
scatter(a3,5*ones(sum(idx),1),opt_r(idx),'filled','markerfacealpha',.1)

legend('lpsp th','empty th','lpsp mcherry','lpsp vglut','empty vglut')
xlim(a2,[0,6]); plot(a2,xlim,[10,10],':k')
xlim(a3,[0,6])
