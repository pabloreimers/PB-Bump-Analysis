...
for i = 1:length(all_files)
    ...
    if ~isfile([fileparts(all_files(i).folder),'\mask.mat'])
        ...
        figure(1); clf; imagesc(mean(imgData,3)*10); colormap(parula); axis equal tight; drawnow;
        mask = roipoly();
...
dc = gradient(unwrap(all_data(i).ft.cue))*60;
m  = unwrap(all_data(i).im.mu);
m(all_data(i).im.rho<rho_thresh) = nan;
m  = interp1(all_data(i).ft.xb,m,all_data(i).ft.xf);
dm = gradient(m) * 60;
...
for i = 1:length(all_data)
    ...
    if strcmp(curr_fly,all_data(i).meta(1:62)) %~any(all_data(i).ft.stims)
        continue
    end
    
    curr_fly = all_data(i).meta(1:62);
    ...
    if contains(all_data(i).meta,'empty') | contains(all_data(i).meta,'+')
        ...
        mobi_empty = [mobi_empty;{all_data(i).gain.m}];
        ...
        mobi_chrim = [mobi_chrim;{all_data(i).gain.m}];
    end
end
...
mobi_empty = cell2mat(mobi_empty);        
gain_chrim = cell2mat(gain_chrim);
fval_chrim = cell2mat(fval_chrim);
bias_chrim = cell2mat(bias_chrim);
dist_chrim = cell2mat(dist_chrim);
mobi_chrim = cell2mat(mobi_chrim);      

%
fval_max = 1e3;
dist_thresh = 7;
figure(8); clf
subplot(3,3,1); hold on; scatter(dist_empty,fval_empty,'filled','MarkerFaceAlpha',.01); 
                         scatter(dist_chrim,fval_chrim,'filled','MarkerFaceAlpha',.01); xlabel('dist'); ylabel('fval'); set(gca,'YScale','log')
subplot(3,3,2); hold on; histogram(dist_empty); plot(dist_thresh*[1,1],ylim,'r'); xlabel('dist'); 
subplot(3,3,3); hold on; histogram(fval_empty,'binwidth',100); plot(fval_max*[1,1],ylim,'r'); xlabel('fval'); set(gca,'XScale','log')
...
idx1 = dist_empty > dist_thresh;
idx2 = dist_chrim > dist_thresh;
histogram(mobi_empty(idx1) ./ dist_empty(idx1),'Normalization','probability','Binwidth',.1)
histogram(mobi_chrim(idx2) ./ dist_chrim(idx2),'Normalization','probability','Binwidth',.1)
legend(sprintf('empty (%d)',num_empty),sprintf('cschrimson (%d)',num_chrim))
xlabel({'bump mobility','baseline (30s window)'})
ylabel('%')
%% show movie
% i = 19;
% pause_time = 1e-2;
...
empty_idx = contains({all_data.meta},'empty')';
chrim_idx = ~empty_idx;
stim_idx = false(size(chrim_idx));
for i = 1:length(all_data)
...
stim_time = zeros(length(all_data),1);
...
for i = 1:length(all_data)    
    ...
    if any(all_data(i).ft.stims)
        stim_time(i) = all_data(i).ft.xf(find(all_data(i).ft.stims,1,'last')) - all_data(i).ft.xf(find(all_data(i).ft.stims,1,'first'));
    end
end

figure(5); clf; subplot(2,1,1); hold on
plot([1,2],[out_stim_r(chrim_idx&stim_idx&stim_time>10),in_stim_r(chrim_idx&stim_idx&stim_time>10)],'.-k')
plot([3,4],[out_stim_r(~chrim_idx&stim_idx&stim_time>10),in_stim_r(~chrim_idx&stim_idx&stim_time>10)],'.-k')
ylabel('average r speed')
xlim([.5,4.5])
xticks([])

subplot(2,1,2); hold on
plot([1,2],[out_stim_f(chrim_idx&stim_idx&stim_time>10),in_stim_f(chrim_idx&stim_idx&stim_time>10)],'.-k')
plot([3,4],[out_stim_f(~chrim_idx&stim_idx&stim_time>10),in_stim_f(~chrim_idx&stim_idx&stim_time>10)],'.-k')
...
%% compare bump properties (amp, width, shallowness before and after stim)
win_size = 60; % how long of a starting or ending bin to average

stim_time = zeros(length(all_data),1);
bump_max_pre = nan(length(all_data),1);
bump_max_post = nan(length(all_data),1);
bump_min_pre = nan(length(all_data),1);
bump_min_post = nan(length(all_data),1);
bump_rho_pre = nan(length(all_data),1);
bump_rho_post = nan(length(all_data),1);
f_speed_pre = nan(length(all_data),1);
f_speed_post = nan(length(all_data),1);

for i = 1:length(all_data)
    
    bump_max_tmp = interp1(all_data(i).ft.xb,max(all_data(i).im.d,[],1),all_data(i).ft.xf);
    bump_min_tmp = interp1(all_data(i).ft.xb,min(all_data(i).im.d,[],1),all_data(i).ft.xf);
    bump_rho_tmp = interp1(all_data(i).ft.xb,all_data(i).im.rho,all_data(i).ft.xf);

    num_frames = round(win_size / mean(diff(all_data(i).ft.xf))); %find number of frames for winsize


    if any(all_data(i).ft.stims)
        stim_time(i)    = all_data(i).ft.xf(find(all_data(i).ft.stims,1,'last')) - all_data(i).ft.xf(find(all_data(i).ft.stims,1,'first'));
        stim_start  = find(all_data(i).ft.stims,1,'first');
        stim_end    = find(all_data(i).ft.stims,1,'last');
        
    else
        stim_start = num_frames+1;
        stim_end = length(all_data(i).ft.xf) - num_frames;
    end

    bump_max_pre(i) = mean(bump_max_tmp(stim_start-num_frames:stim_start),'omitnan');
    bump_min_pre(i) = mean(bump_min_tmp(stim_start-num_frames:stim_start),'omitnan');
    bump_rho_pre(i) = mean(bump_rho_tmp(stim_start-num_frames:stim_start),'omitnan');
    tmp = all_data(i).ft.f_speed(stim_start-num_frames:stim_start);
    tmp = tmp(tmp>.5);
    f_speed_pre(i)  = mean(tmp);

    bump_max_post(i) = mean(bump_max_tmp(stim_end:stim_end+num_frames),'omitnan');
    bump_min_post(i) = mean(bump_min_tmp(stim_end:stim_end+num_frames),'omitnan');
    bump_rho_post(i) = mean(bump_rho_tmp(stim_end:stim_end+num_frames),'omitnan');
    tmp = all_data(i).ft.f_speed(stim_end:stim_end+num_frames);
    tmp = tmp(tmp>.5);
    f_speed_post(i)  = mean(tmp);
end
...
scatter(empty_idx(stim_time==0),bump_max_pre(stim_time==0))
plot(xlim,[0,0],':k')
title('bump height (no stim)'); xlim([-.5,1.5])

subplot(3,3,2); hold on
scatter(empty_idx(stim_time>10),bump_max_post(stim_time>10) - bump_min_pre(stim_time>10))
plot(xlim,[0,0],':k')
title('bump height change (stim > 10)'); xlim([-.5,1.5])

subplot(3,3,3); hold on
scatter(empty_idx(stim_time>10),bump_min_post(stim_time>10) - bump_min_pre(stim_time>10))
plot(xlim,[0,0],':k')
title('bump min change (stim > 10)'); xlim([-.5,1.5])

subplot(3,3,4); hold on
scatter(empty_idx(stim_time<10 & stim_time>0),bump_min_post(stim_time<10 & stim_time>0) - bump_min_pre(stim_time<10 & stim_time>0))
plot(xlim,[0,0],':k')
title('bump min change (stim<10)'); xlim([-.5,1.5])

subplot(3,3,5); hold on
scatter(empty_idx(stim_time>10),bump_rho_post(stim_time>10) - bump_rho_pre(stim_time>10))
plot(xlim,[0,0],':k')
plot(xlim,[0,0],':k')
title('rho change (stim >10)'); xlim([-.5,1.5])

subplot(3,3,6); hold on
scatter(empty_idx(stim_time<10 & stim_time>0),bump_rho_post(stim_time<10 & stim_time>0) - bump_rho_pre(stim_time<10 & stim_time>0))
plot(xlim,[0,0],':k')
title('rho change (stim<10)'); xlim([-.5,1.5])

subplot(3,3,7); hold on
scatter(empty_idx(stim_time>10),f_speed_pre(stim_time>10))
plot(xlim,[0,0],':k')
title('f speed pre (stim>10)'); xlim([-.5,1.5])

subplot(3,3,8); hold on
scatter(empty_idx(stim_time>10),f_speed_post(stim_time>10))
plot(xlim,[0,0],':k')
title('f speed post (stim>10)'); xlim([-.5,1.5])

subplot(3,3,9); hold on
scatter(empty_idx(stim_time>10),f_speed_post(stim_time>10) - f_speed_pre(stim_time>10))
plot(xlim,[0,0],':k')
title('f speed change (stim>10)'); xlim([-.5,1.5])
%% Functions
