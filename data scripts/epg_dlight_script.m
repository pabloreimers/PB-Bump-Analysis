...
for i = 1:length(all_files)
    ...
    if ~isfile([fileparts(all_files(i).folder),'\mask.mat'])
        ...
        figure(1); clf; imagesc(r_corr); colormap(bone); axis equal tight; drawnow;
        mask = roipoly();
...
scatter(cr(~freeze_idx & ~high_idx),amp(~freeze_idx & ~high_idx),5,'k','filled','MarkerFaceAlpha',.1)
scatter(cr(freeze_idx),amp(freeze_idx),5,'r','filled','MarkerFaceAlpha',.1)
scatter(cr(high_idx),amp(high_idx),5,'c','filled','MarkerFaceAlpha',.1)
legend('moving','frozen','high')
...
errorbar(edges(1:end-1),binned_amp(:,3),binned_sem(:,3),'o-c')
...
%% for GRAB-DA with complicated gain blocks. show binned amp vs rotational speed for every trial
pre_smooth = 90;
post_smooth = 90; %these are in units of frames. so it's really 2s 
...
for j = 1:(length(edges)-1)    
    idx1 = cm < 10 & cr > edges(j) & cr < edges(j+1) & g < .5;
    idx2 = cm < 10 & cr > edges(j) & cr < edges(j+1) & g > .5 & g < 1.0;
    idx3 = cm < 10 & cr > edges(j) & cr < edges(j+1) & g > 1;
    ...
    binned_amp(j,1,i) = mean(amp(idx1),'omitnan');
    binned_amp(j,2,i) = mean(amp(idx2),'omitnan');
    binned_amp(j,3,i) = mean(amp(idx3),'omitnan');
    binned_sem(j,1,i) = std(amp(idx1),'omitnan') / sqrt(sum(~isnan(amp(idx1))));
    binned_sem(j,2,i) = std(amp(idx2),'omitnan') / sqrt(sum(~isnan(amp(idx2))));
    binned_sem(j,3,i) = std(amp(idx3),'omitnan') / sqrt(sum(~isnan(amp(idx3))));
end
...
%if i > 1 && ic(i) > ic(i-1); nexttile; hold on; end 
nexttile; hold on
errorbar(edges(1:end-1),binned_amp(:,1,i),binned_sem(:,1,i),'-','Color',[.5,0,0])
errorbar(edges(1:end-1),binned_amp(:,2,i),binned_sem(:,2,i),'-','Color',[1,.5,0])
errorbar(edges(1:end-1),binned_amp(:,3,i),binned_sem(:,3,i),'-','Color',[1,0,0])
%title(sprintf('gain: %.2f',g))
end
...
edges = 0:.25:4;
binned_amp = nan(length(edges)-1,4,max(fly_id));
binned_sem = nan(length(edges)-1,4,max(fly_id));

amp_cell = cell(length(all_data),1);
cr_cell  = cell(length(all_data),1);
cc_cell  = cell(length(all_data),1);
cm_cell  = cell(length(all_data),1);
g_cell  = cell(length(all_data),1);
...
dm  = smoothdata([diff(mu) * 60;0],'gaussian',1);
cm_cell{i}  = smoothdata(abs(dm),'movmean',[post_smooth,0]); %calculate the cumulative rotational speed
g_cell{i}   = round(medfilt1(-dc ./ dr,1e3),1);
...
cc = cat(1,cc_cell{fly_id == i & ~dark_idx});
cm = cat(1,cm_cell{fly_id == i & ~dark_idx});
g  = cat(1,g_cell{fly_id == i & ~dark_idx});
...
for j = 1:(length(edges)-1)    
    %idx1 = cm < 10 & cr > edges(j) & cr < edges(j+1) & g < .5;
    idx1 = cm < 10 & cr > edges(j) & cr < edges(j+1) & freeze_idx;
    idx2 = cm < 10 & cr > edges(j) & cr < edges(j+1) & ~freeze_idx & ~high_idx;
    idx3 = cm < 10 & cr > edges(j) & cr < edges(j+1) & high_idx;
...
for j = 1:(length(edges)-1)    
    ...
    binned_amp(j,4,i) = mean(amp(idx1),'omitnan');
    binned_sem(j,4,i) = std(amp(idx1),'omitnan') / sqrt(sum(~isnan(amp(idx1))));
end
...
%if i > 1 && ic(i) > ic(i-1); nexttile; hold on; end 
nexttile; hold on
errorbar(edges(1:end-1),binned_amp(:,1,i),binned_sem(:,1,i),'-','Color',[.5,0,0],'Linewidth',2)
errorbar(edges(1:end-1),binned_amp(:,2,i),binned_sem(:,2,i),'-','Color',[1,.5,0],'Linewidth',2)
errorbar(edges(1:end-1),binned_amp(:,3,i),binned_sem(:,3,i),'-','Color',[1,0,0],'Linewidth',2)
errorbar(edges(1:end-1),binned_amp(:,4,i),binned_sem(:,4,i),'-','Color',[0,0,1],'Linewidth',2)
