...
for i = 1:length(all_data)
    ...
    tmp1 = m_speed(lag+1:end);
    tmp2 = c_speed(1:end-lag);
    tmp3 = rho_tmp(lag+1:end);
    idx  = abs(tmp1) < 10 & abs(tmp2) > .1 & tmp3 > .2;
    
    tmp1(~idx) = nan;
    tmp2(~idx) = nan;
    num_frames = ceil(win/fr);
    max_frames = length(tmp1);
    tmp_gains  = nan(1,max_frames);
    for j = 1:max_frames
        tmp11 = tmp1(j:min(j+num_frames,max_frames));
        tmp21 = tmp2(j:min(j+num_frames,max_frames));
        idx2 = ~isnan(tmp11);
        if sum(idx2)>2
            tmp_gains(j) = tmp11(idx2) \ -tmp21(idx2);
        end
    end
    gains{i} = tmp_gains;
end
...
for i = 1:length(idx)
    ...
    plot(all_data(idx(i)).ft.xf,-all_data(idx(i)).ft.cue)
    plot(all_data(idx(i)).ft.xb,all_data(idx(i)).im.mu)
    plot(xlim,[.8,.8],'r:'); ylim([-.1,2])
end
...
for i = 1:length(all_data)
    ...
    for j = loc
        counter = counter+1;
        [~,k] = min(abs(all_data(i).ft.xf - all_data(i).ft.xb(j)));
        g_pulses{counter} = gains{i}(min(max(k+tmp_win2,1),length(gains{i})));
        dark_idx{counter} = contains(all_data(i).meta,'dark');
        exp_idx{counter} = ~contains(all_data(i).meta,'control');
    end
end
...
figure(12); clf
tmp1 = cell2mat(g_pulses(exp_idx)');
h = plot_sem(gca,(1:size(tmp1,2))',tmp1); h.FaceColor = 'r'; ylim([-1,1])
hold on
tmp2 = cell2mat(g_pulses(~exp_idx)');
h = plot_sem(gca,(1:size(tmp2,2))',tmp2); h.FaceColor = 'b'; ylim([-1,1])
...
for i = 1:length(all_data)
    ...
    tmp1 = mu_tmp(lag+1:end);
    tmp2 = r_speed(1:end-lag);
    tmp3 = rho_tmp(lag+1:end);
    idx  = abs(tmp2) > .1 & tmp3 > .2;
    
    tmp1(~idx) = nan;
    %tmp2(~idx) = nan;
    num_frames = ceil(win/fr);
    max_frames = length(tmp1);
    tmp_frames = 1:30:max_frames;
    tmp_gains  = nan(length(tmp_frames),2);
    tmp_fvals  = nan(length(tmp_frames),1);
    for k = 1:length(tmp_frames)
        j = tmp_frames(k);
        tmp11 = tmp1(j:min(j+num_frames,max_frames));
        tmp21 = tmp2(j:min(j+num_frames,max_frames));
        idx2 = ~isnan(tmp11);
        if sum(idx2)>2
            [tmp_gains(k,:),tmp_fvals(k)] = fmincon(@(x)(sum((x(1)*fr*cumsum(tmp21) - (tmp11 - x(2))).^2,'omitnan')),x0,[],[],[],[],lb,ub,[],opts);
        end
    end
    gains_int{i} = tmp_gains;
...
for i = 1:length(all_data)
    ...
    tmp_win2= round(tmp_win2/30);
    for j = loc
        counter = counter+1;
        [~,k] = min(abs(all_data(i).ft.xf(1:30:end) - all_data(i).ft.xb(j)));
        g_pulses{counter} = gains_int{i}(min(max(k+tmp_win2,1),length(gains_int{i})),1)';
        v_pulses{counter} = fvals_int{i}(min(max(k+tmp_win2,1),length(gains_int{i})));
        dark_idx{counter} = contains(all_data(i).meta,'dark');
        exp_idx{counter} = ~contains(all_data(i).meta,'control');
    end
end
...
figure(12); clf
tmp1 = cell2mat(g_pulses(exp_idx&dark_idx)');
tmp3  = cell2mat(v_pulses(exp_idx&dark_idx))';
tmp1(tmp3>5e3) = nan;
h = plot_sem(gca,(1:size(tmp1,2))',tmp1); h.FaceColor = 'r'; %ylim([-1,3])
hold on
tmp2 = cell2mat(g_pulses(exp_idx&~dark_idx)');
tmp3  = cell2mat(v_pulses(exp_idx&~dark_idx))';
tmp2(tmp3>5e3) = nan;
h = plot_sem(gca,(1:size(tmp2,2))',tmp2); h.FaceColor = 'b';% ylim([-1,3])
...
m1 = mean(x,1,'omitnan');
s1 = std(x,1,'omitnan')./sqrt(size(x,1));
