...
%% plot heading traces
idx = find(cellfun(@(x)(contains(x,'20260514\fly 3')),{all_data.meta})); %,6,'last');
dark_mode = true;
figure(2); clf
...
figure(5); clf
idx = dark_idx & con_idx;
tmp1 = cell2mat(gains_inst(idx)');
tmp2 = cell2mat(disp_inst(idx)');
tmp3 = cell2mat(stims_inst(idx))';
histogram(tmp1(tmp2>dist_min & tmp3<stims_max),'binwidth',.1,'Normalization','probability')
...
win_gain  = 10;
r_min = .1;
pulse_gains = cell(length(all_data),1);

for i = 1:length(all_data)
    i
    stims = interp1(all_data(i).ft.xb,sum(all_data(i).atp.d,1),all_data(i).ft.xf);
    mu_tmp = interp1(all_data(i).ft.xb,unwrap(all_data(i).im.mu),all_data(i).ft.xf);
    m_speed = gradient(mu_tmp) / mean(diff(all_data(i).ft.xf));
    r_speed = all_data(i).ft.r_speed;


    [~,loc_start] = findpeaks(stims,'MinPeakProminence',9);
    if isempty(loc_start); continue; end
    loc_end = [loc_start(2:end);length(stims)];
    win_frames = ceil(win_gain / mean(diff(all_data(i).ft.xf))); %find the number of frames in rolling gain window
    gain_tmp = nan(length(m_speed));
    
    for j = 1:length(m_speed)
        end_idx = min(j+win_frames,length(r_speed));
        r_tmp = r_speed(j:min(j+win_frames,length(r_speed))); 
        m_tmp = m_speed(j:min(j+win_frames,length(r_speed)));
        idx = abs(r_tmp) > r_min & ~isnan(r_tmp) & ~isnan(m_tmp);
        if ~any(idx); continue; end
        gain_tmp(j) = r_tmp(idx) \ m_tmp(idx);
    end
   
    pulse_frames = ceil(win_pulse / mean(diff(all_data(i).ft.xf))); %find the number of frames in rolling gain window
    gain_mat = nan(length(loc),pulse_frames);
    for j = 1:length(loc_start)
        gain_mat(j,1:(loc_end(j)-loc_start(j)+1)) = gain_tmp(loc_start(j):loc_end(j));
    end
    pulse_gains{i} = gain_mat;
end
...
plot(mean(tmp,1,'omitnan') + std(tmp,1,'omitnan')./sum(~isnan(tmp),1),'k')
plot(mean(tmp,1,'omitnan') - std(tmp,1,'omitnan')./sum(~isnan(tmp),1),'k')
