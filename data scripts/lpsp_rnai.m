...
for i = 1:length(all_data)
    ...
    tmp = tmp(abs(all_data(i).ft.r_speed)>1);
    o{i} = tmp(~isnan(tmp));
    h{i} = circ_var(all_data(i).ft.cue);
end
o = reshape(o,[],1);
h = reshape(h,[],1);
...
figure(5); clf
subplot(2,2,1)
idx = ~dark_idx & walk_idx;
scatter(empty_idx(idx),var_h(idx))
axis padded
title('heading variance')

subplot(2,2,3)
empty_ind = find(empty_idx & idx); %find indices to each group
lpsp_ind  = find(~empty_idx & idx);
n = 1e4;
...
hold on
swarmchart(ones(n,1),mean(var_h(empty_mat),1),'.','Color',[0,.5,1])
swarmchart(zeros(n,1),mean(var_h(lpsp_mat),1),'.','Color',[1,.5,0])
axis padded

p = sum((mean(var_h(empty_mat),1) - mean(var_h(lpsp_mat),1))>0) / n; %count how many times the mean of the empty group is higher than the mean of the lpsp group, divide by n for p value
title(sprintf('p: %.3f',p))

subplot(2,2,2)
idx = ~dark_idx & walk_idx;
scatter(empty_idx(idx),var_o(idx))
axis padded
title('offset variance')

subplot(2,2,4)
hold on
swarmchart(ones(n,1),mean(var_o(empty_mat),1),'.','Color',[0,.5,1])
swarmchart(zeros(n,1),mean(var_o(lpsp_mat),1),'.','Color',[1,.5,0])
axis padded

p = sum((mean(var_o(empty_mat),1) - mean(var_o(lpsp_mat),1))>0) / n; %count how many times the mean of the empty group is higher than the mean of the lpsp group, divide by n for p value
title(sprintf('p: %.3f',p))
