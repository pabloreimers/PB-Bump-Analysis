%% load in data
base_dir = 'Z:\pablo\epg_grab(da2m)_jrgeco1a\'; %uigetdir(); %
...
for i = length(all_data):length(all_files)
    ...
    if isfile([fileparts(all_files(i).folder),'\pablo_ft.mat'])
        ...
        img{2} = rot90(img{2},2);
        mask = rot90(mask,2);
        if mean(ft.cue_brightness) > 0; all_data(i).ft.pattern = 'cl_bar'; else; all_data(i).ft.pattern = 'background'; end
    else
    ...
    all_data(i).grab = process_im(squeeze(sum(img{1},3)), im_win, im_type, mask, n_centroid, f0_pct);
    all_data(i).geco = process_im(squeeze(sum(img{2},3)), im_win, im_type, mask, n_centroid, f0_pct);
...
a = plot(all_data(i).ft.xb,all_data(i).grab.mu,'k','linewidth',2); a.YData(abs(diff(a.YData))>pi) = nan;

cmap =  [linspace(1,0,255)',linspace(1,.75,255)',linspace(1,0,255)'];
set(gca,'Colormap',cmap,'CLim',clim+[0,-0.5]*range(clim))
ylabel('GRAB(DA2m)')
title(fileparts(all_data(i).meta))
...
mu  = interp1(all_data(i).ft.xb,unwrap(all_data(i).grab.mu),all_data(i).ft.xf);
offset1 = circ_dist(mu(idx),-all_data(i).ft.cue(idx));
histogram(offset1,-pi:.1:pi,'Orientation','horizontal','edgeColor','none','Normalization','probability','FaceColor',[0,.75,0])
box(ax,'off')
ax.YAxisLocation =  'right'; ax.YLim = [-pi,pi]; ax.YTick = [-pi,0,pi]; ax.YTickLabels = {'-\pi','0','\pi'};

subplot(3,1,2)
imagesc(all_data(i).ft.xb,unwrap(all_data(i).grab.alpha),all_data(i).geco.z)
hold on
if contains(all_data(i).ft.pattern,'background'); c = 'm'; else; c = 'c'; end
a = plot(all_data(i).ft.xf,-all_data(i).ft.cue,c,'linewidth',2); a.YData(abs(diff(a.YData))>pi) = nan;
a = plot(all_data(i).ft.xb,all_data(i).geco.mu,'k','linewidth',2); a.YData(abs(diff(a.YData))>pi) = nan;

cmap =  [linspace(1,.75,255)',linspace(1,0,255)',linspace(1,0,255)'];
set(gca,'Colormap',cmap,'CLim',clim+[0.1,-.6]*range(clim))
ylabel('jRGECo1a')
...
ax = axes('Position',pos,'Color','none','XAxisLocation','top'); hold on
idx = all_data(i).ft.r_speed > r_thresh;
mu  = interp1(all_data(i).ft.xb,unwrap(all_data(i).geco.mu),all_data(i).ft.xf);
offset2 = circ_dist(mu(idx),-all_data(i).ft.cue(idx));
histogram(offset2,-pi:.1:pi,'Orientation','horizontal','edgeColor','none','Normalization','probability','FaceColor',[.75,0,0])
box(ax,'off')
ax.YAxisLocation =  'right'; ax.YLim = [-pi,pi]; ax.YTick = [-pi,0,pi]; ax.YTickLabels = {'-\pi','0','\pi'};
...
subplot(3,1,3)
tmp = ones([size(all_data(i).geco.z),3]);
tmp(:,:,2:3) = tmp(:,:,2:3) - all_data(i).geco.d;
tmp(:,:,[1,3]) = tmp(:,:,[1,3]) - all_data(i).grab.d;

image(all_data(i).ft.xb,unwrap(all_data(i).geco.alpha),tmp)
hold on
a = plot(all_data(i).ft.xb,all_data(i).grab.mu,'Color',[0,.75,0],'linewidth',2); a.YData(abs(diff(a.YData))>pi) = nan;
a = plot(all_data(i).ft.xb,all_data(i).geco.mu,'Color',[.75,0,0],'linewidth',2); a.YData(abs(diff(a.YData))>pi) = nan;
...
%% 
m = nan(length(all_data),2);
s = nan(length(all_data),2);
o = nan(length(all_data),2);


figure(2); clf
tiledlayout("flow")
for i = 1:length(all_data)
    %if contains(all_data(i).ft.pattern,'background'); continue; end
    mu1  = interp1(all_data(i).ft.xb,unwrap(all_data(i).grab.mu),all_data(i).ft.xf);
    mu2  = interp1(all_data(i).ft.xb,unwrap(all_data(i).geco.mu),all_data(i).ft.xf);
    rho  = interp1(all_data(i).ft.xb,all_data(i).geco.rho,all_data(i).ft.xf);
    idx = all_data(i).ft.r_speed > r_thresh & ~isnan(mu1) & ~isnan(mu2) & rho > .1;
    
    
    offset1 = circ_dist(mu1(idx),-all_data(i).ft.cue(idx));
    m(i,1) = circ_mean(offset1);
    s(i,1) = circ_var(offset1)/sqrt(length(offset1));

    offset2 = circ_dist(mu2(idx),-all_data(i).ft.cue(idx));
    m(i,2) = circ_mean(offset2);
    s(i,2) = circ_var(offset2)/sqrt(length(offset2));
    
    nexttile; hold on
    histogram(offset1,-pi:.1:pi,'edgeColor','none','Normalization','probability','FaceColor',[0,.75,0])
    histogram(offset2,-pi:.1:pi,'edgeColor','none','Normalization','probability','FaceColor',[.75,0,0])

    o(i,1) = circ_mean(circ_dist(mu1(idx),mu2(idx)));
    o(i,2) = circ_var(circ_dist(mu1(idx),mu2(idx))) / sqrt(sum(idx));
end

figure(3); clf; hold on
errorbar(1:length(all_data),m(:,1),s(:,1),'og')
errorbar(1:length(all_data),m(:,2),s(:,2),'or')
errorbar(1:length(all_data),circ_dist(m(:,1),m(:,2)),sum(s,2),'ok')
errorbar(1:length(all_data),o(:,1),o(:,2),'om')
%% Functions
