...
for i = 1:length(offset_var)
    tmp = ~isnan(full_data.mu{i});
    offset_var(i) = var(circ_dist(full_data.mu{i}(tmp),-full_data.cue{i}(tmp)));
end
...
p = sum(mean(exp_resamp,1) - mean(con_resamp,1) > 0) / N;
