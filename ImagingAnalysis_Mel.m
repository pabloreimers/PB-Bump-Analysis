...
c_speed = [diff(unwrap(cue/192*2*pi));0]*60;
c_speed(abs(c_speed)>2) = 0;
...
c_speed = smoothdata(c_speed,1,'gaussian',f_smooth);
...
hold on; plot(xf,c_speed,'b','linewidth',2)
yyaxis right; plot(xf, amp,'Color',c1,'linewidth',2); ylabel('Average \DeltaF/F'); ax = gca; ax.YAxis(2).Color = c1;
xlabel('time (s)')
axis tight
% y = ylim; text(xb(end),y(2),sprintf('r^2: %.2f\n2p lag: %.1fs\njoint r^2:%.2f',...
...
%                 'HorizontalAlignment','right','VerticalAlignment','top')
y = ylim; text(xf(end),y(2),sprintf('r^2: %.2f\njoint r^2: %.2f\nOL r^2: %.2f',...
                                r_gof.adjrsquare,m_gof.adjrsquare,c_gof.adjrsquare),...
                'HorizontalAlignment','right','VerticalAlignment','top')
...
x = xlim;
y = ylim;
text(x(2),y(2),sprintf('$r = %.2f$\n$b = %.2f$',vel_rho,b(2)),'Interpreter','latex','HorizontalAlignment','right','VerticalAlignment','top')
plot([0,0],y,'k:')
plot(x,[0,0],':k')
plot(x,x*b(2)+b(1),'r')
%end
...
%% show dff and fly heading together
figure(15); clf
subplot(3,1,1); hold on
imagesc(xf,alpha+[zeros(1,20),2*pi*ones(1,20)],dff_cluster); axis tight
a = plot(xf,-cue,'m'); a.YData(abs(diff(a.YData))>pi) = nan;
a = plot(xf,-cue+2*pi,'m'); a.YData(abs(diff(a.YData))>pi) = nan;

%% see open loop component
still_idx = r_speed < 1e-1;
figure(4); clf
subplot(2,2,1)
scatter(c_speed(still_idx),amp(still_idx),'filled','MarkerFaceAlpha',.2)
xlabel('OL speed'); ylabel('dF/F')
text(max(c_speed(still_idx)),min(amp(still_idx)),'while fly is stopped','VerticalAlignment','bottom','HorizontalAlignment','right')

subplot(2,2,2)
scatter(r_speed,amp,[],c_speed,'filled','markerfacealpha',.1)
xlabel('fly speed'); ylabel('dF/F')
tmp = colorbar;
ylabel(tmp,'OL speed')


tmp1 = round(c_speed*10)+1;
tmp3 = accumarray(tmp1(still_idx),amp(still_idx),[],@mean,nan); %,[size(tmp1),size(tmp2)],@mean);
tmp3(isnan(tmp3)) = [];
subplot(2,2,3)
plot((unique(tmp1(still_idx))-1)/10,tmp3)
